    ...
    <div className="container" style={{ color: "#fff" }}>
      ...
      <div className="card mb-3 bg-dark">
        <div className="row g-0">
          <button className="btn btn-light mb-3" onClick={back}>
            Go Back
          </button>
          <div className="col-md-7">
            <div className="card-body">
              <div className="row">
                <div className="col-2" style={{ display: "flex" }}>
                  <img
                    ...
                    src={`https://image.tmdb.org/t/p/w500${singleDate?.poster_path}`}
                    alt="images"
                  />
                </div>
                <div className="col-8" style={{ textAlign: "start" }}>
                  <h5>{singleDate?.title}</h5>
                  <p style={{ color: "rgb(111 122 143)" }}>
                    Rating:{singleDate?.vote_average}
                  </p>
                  <p style={{ color: "rgb(111 122 143)" }}>
                    <span className="badge bg-secondary">
                      {singleDate?.runtime}
                    </span>{" "}
                    {singleDate?.genres?.map((res) => `${res.name},`)}
                  </p>
                  <h6 style={{ fontSize: "12px" }}>
                    Release Date: {singleDate?.release_date}
                  </h6>
                </div>
              </div>
              <br />
              <div style={{ textAlign: "start" }}>
                ...
                <h6 className="card-text" style={{ fontSize: "12px" }}>
                  {singleDate?.overview}
                </h6>
          ...
          <div className="col-md-5">
            <img
              style={{ float: "right" }}
              src={`https://image.tmdb.org/t/p/w500${singleDate.backdrop_path}`}
      ...
      <div className="row">
        {casting?.map((res) => (
          ...
          >
            <div
              className="card"
              style={{ width: "11rem", backgroundColor: "rgb(32, 32, 32)" }}
            >
              {res?.profile_path === null ? (
                <img src="/gol.png" className="card-img-top" alt="..." />
              ) : (
                <img
